...
CREATE TABLE IF NOT EXISTS public.pos_receipts_today (
...
CREATE UNIQUE INDEX IF NOT EXISTS uidx_pos_receipts_today_brand_order
  ON public.pos_receipts_today (brand_id, order_id);

CREATE INDEX IF NOT EXISTS idx_pos_receipts_today_brand_issued
  ON public.pos_receipts_today (brand_id, issued_at DESC);
...
BEGIN
  ...
  WHERE r.brand_id = v_staff.brand_id
    AND (r.issued_at AT TIME ZONE 'UTC')::date = current_date
