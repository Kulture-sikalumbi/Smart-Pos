...
BEGIN
  ...
  -- IMPORTANT:
  -- tablet orders are created as OPEN and NOT sent_to_kitchen.
  -- cashier reviews first, then explicitly sends kitchen-prepared items.
  ...
  VALUES (
    v_order_id, v_assignment.brand_id, v_order_no, v_assignment.table_no, 'eat_in', 'open',
    'tablet', format('Tablet Table %s', v_assignment.table_no), v_subtotal, 0, 0, v_tax, v_total,
    v_total_cost, v_gross_profit, v_gp_percent, now(), NULL, 'tablet', v_key
  );
